...
import { setupPostgresDatabaseAndSetEnv, setGuardTypes, truncateDB } from "../Helper";
...
describe('Images (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        const [psql, { localstack: ls, s3: _s3 }] = await Promise.all([setupPostgresDatabaseAndSetEnv(), createLocalstackWithS3()]);
    ...
    beforeEach(async () => {
        const moduleFixture = await Test.createTestingModule({
            imports: [AppModule],
        }).overrideProvider(S3Client).useValue(s3).compile();
    ...
    it('User can upload image', async () => {
        ...
        expect(result.body.url).toMatch(/^([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})-([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\.jpg$/i);
    })

    it('User can get uploaded image', async () => {
        const { body: { url } } = await client.uploadImage('1');
    ...
    it('User can grant access to image for other users', async () => {
        const { body: { id: grantee } } = await auth.whoami('2')
        const { body: { url } } = await client.uploadImage('1');
